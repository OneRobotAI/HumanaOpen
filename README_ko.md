...
`q` / Ctrl+C로 추론을 중지하면 스크립트는:
1. **베이스 바퀴를 정지**(마지막으로 `x.vel=0 / theta.vel=0` 전송)하고 리프트를 현재
   높이에 유지
2. 리프트 위치 저장
3. **"Press ENTER to release torque and disconnect..."** 프롬프트
4. ENTER 전에 팔을 잡으세요 — 토크가 해제되면 팔이 자유롭게 떨어집니다
5. 준비되면 ENTER → 연결 해제

이렇게 하면 서보 전원이 꺼질 때 팔이 갑자기 떨어지는 것을 방지합니다.

**베이스/리프트 자동 정지 안전망 (듀얼 머신)**: Host는 워치독을 실행합니다 —
`watchdog_timeout_ms`(500ms) 동안 동작 명령이 없으면 바퀴 속도를 0으로 만들고
리프트를 유지합니다. 따라서 PC 프로세스가 갑작스럽게 종료(`kill -9`)되거나 베이스가
움직이는 중에 연결이 끊겨도 Host가 정지될 때까지 계속 달리는 것이 아니라 베이스가
스스로 정지합니다.

## 로드맵

계획된 방향 (커뮤니티 기여 환영 — [CONTRIBUTING.md](CONTRIBUTING.md) 참조):

### 시뮬레이션 및 모델링
- **URDF / XACRO** — RViz, Gazebo, MuJoCo용 전체 로봇 설명
  (현재 [`hardware/urdf/`](hardware/urdf/)에 템플릿 스켈레톤; 메시와 팔 체인은
  완성 예정).
- **Sim-to-Real** — 시뮬레이션에서 정책 훈련 후 실제 하드웨어로 전이.

### 조작
- **비전 기반 파지** — 클래식 CV(ArUco/분할) + 두 팔을 이용한 집기-놓기.
- **역운동학(IK)** — 7-DOF 팔을 위한 해석적/수치 IK (엔드이펙터 자세 직접 지령).

### 내비게이션
- **베이스 자율 내비게이션** — 차동 구동 베이스를 위한 SLAM / 오도메트리 +
  move_base 스타일 경로 계획.
- **장애물 회피** 및 웨이포인트 추종.

### 상호작용 및 구현형 AI
- **구현형 물리 모델** — 기존 파이프라인(ACT/SmolVLA)을 물리 인지 모델 및
  플랫폼(MuJoCo, Isaac Lab 등)에 적용.
- **VLA / 멀티모달** — 기존 SmolVLA 지원 위에 더 풍부한 언어 조건화 동작 확장.
- **원격 조작 강화** — 힘 피드백, VR/3D 뷰어(rerun/foxglove 이미 지원),
  웹 기반 원격 제어.

### 데이터 및 도구
- **데이터 수집 개선** — 더 풍부한 에피소드 메타데이터, 자동 품질 검사,
  데이터셋 버전 관리.
- **더 큰 커뮤니티 데이터셋** 및 사전 훈련된 체크포인트.
