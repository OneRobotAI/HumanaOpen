...
try:
    ...
    bit2 = (phase >> 2) & 1
    print(f"Phase register (address 18) = 0x{phase:X} = 0b{phase:08b}")
    print(f"BIT2 = {bit2}  ->  speed unit = {'0.0146 RPM/raw' if bit2 else '0.732 RPM/raw'}")

    if bit2:
        # 0.0146 unit: needs 56.25rpm -> v_max ~= 3853
        print(f"  For target 7.5mm/s (300mm/40s): v_max ~= 3853")
    else:
        # 0.732 unit: needs 56.25rpm -> v_max ~= 77
        print(f"  For target 7.5mm/s (300mm/40s): v_max ~= 77")
